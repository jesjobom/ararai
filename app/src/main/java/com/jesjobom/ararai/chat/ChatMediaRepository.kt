...
interface ChatMediaRepository {
    fun createDraftFile(prefix: String, suffix: String): File
    fun deleteDraft(uri: String, persistedUris: Set<String>)
    fun deleteUnreferenced(candidateUris: Set<String>, persistedUris: Set<String>)
    fun reconcile(persistedUris: Set<String>, maxFiles: Int = DEFAULT_RECONCILIATION_FILE_LIMIT)
}

class FileChatMediaRepository(
    private val mediaDir: File,
) : ChatMediaRepository {
    override fun createDraftFile(prefix: String, suffix: String): File {
    ...
    override fun deleteDraft(uri: String, persistedUris: Set<String>) {
        deleteUnreferenced(setOf(uri), persistedUris)
    }

    override fun deleteUnreferenced(candidateUris: Set<String>, persistedUris: Set<String>) {
        val referencedFiles = persistedUris.mapNotNullTo(mutableSetOf()) { ownedFile(it) }
        candidateUris.asSequence()
            .mapNotNull(::ownedFile)
            .filterNot(referencedFiles::contains)
            .forEach(File::delete)
    }

    override fun reconcile(persistedUris: Set<String>, maxFiles: Int) {
        require(maxFiles >= 0)
        if (!mediaDir.isDirectory || maxFiles == 0) return
        val referencedFiles = persistedUris.mapNotNullTo(mutableSetOf()) { ownedFile(it) }
        mediaDir.listFiles().orEmpty().asSequence()
            .filter(File::isFile)
    ...
    private fun ownedFile(uri: String): File? {
        ...
        val rawPath = if (uri.startsWith("file://")) {
